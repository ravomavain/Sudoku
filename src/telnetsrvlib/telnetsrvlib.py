...
Various settings can affect the operation of the server:
    ...
    COMMANDS     = Dictionary of supported commands
                   Key = command (Must be lower case)
...
# --------------------------- Environment Setup ----------------------------
    ...
    def __init__(self, request, client_address, server):
        ...
        for k in dir(self):
            ...
            for alias in getattr(method, "aliases", []):
                self.COMMANDS[alias.lower()] = self.COMMANDS[name]
    ...
    def _ticap_to_str(self, capname):
        res = curses.tigetstr(capname)
        if res is None:
            return ''
    ...
    def setterm(self, term):
        ...
        self.CODES['DEOL'] = self._ticap_to_str('el')
        self.CODES['DEL'] = self._ticap_to_str('dch1')
        self.CODES['INS'] = self._ticap_to_str('ich1')
        self.CODES['CSRLEFT'] = self._ticap_to_str('cub1')
        self.CODES['CSRRIGHT'] = self._ticap_to_str('cuf1')
...
# ---------------------------- Input Functions -----------------------------
    ...
    def readline(self, echo=None, prompt='', use_history=True):
        ...
        while True:
            ...
            if c == theNULL:
                ...
                log.debug('Unsupported control char: %d', ord(c))
            else:
                if ord(c) < 32:
    ...
    def ungetc(self, char, block=True):
        ...
        # This is very different between green threads and real threads.
        raise NotImplementedError("Please Implement the ungetc method")
...
# --------------------------- Output Functions -----------------------------
    ...
    def writecooked(self, text):
        ...
        self.sock.sendall(bytes(text, "latin-1"))
    ...
    def _inputcooker_getc(self, block=True):
        ...
        if not block:
            if not self.inputcooker_socket_ready():
                return ''
        ret = str(self.sock.recv(1024), "latin-1")
        self.eof = not(ret)
                ...
                if not self.iacseq:
                    if c == IAC:
                        ...
                        if c2 == theNULL or c2 == '':
                            ...
                            c = chr(10)
                    elif c in (x[0] for x in self.ESCSEQ):
                        'Looks like the begining of a key sequence'
                        codes = c
                        for keyseq in self.ESCSEQ:
                            if len(keyseq) == 0:
                                continue
...
# Line 2+: Long descriptive text. (Can be blank line)
    ...
    def cmdHELP(self, params):
        ...
        for cmd in sorted(self.COMMANDS):
            ...
            if getattr(method, 'hidden', False) or getattr(method, 'command_name', '') != cmd:
                continue
    ...
    @command(['exit', 'quit', 'logout', 'logoff'])
        ...
        while self.RUNSHELL:
            ...
            if self.input.cmd:
                ...
                if cmd in self.COMMANDS:
                    ...
                    self.writeerror("Unknown command '%s'. Type 'help' for a list of commands" % cmd)
